...
/// LED pattern supported tpes.
typedef SWIFT_ENUM(NSInteger, NotificationLEDPatternType) {

/// LED emits light in a fully lit - dim pattern, creating a flashing or blinking effect.
  NotificationLEDPatternTypeLEDBlink = 0,

/// LED slowly brightens followed by a slowly dimming pattern, creating a modulating or waving efftect.
...
/// \param failure A completion handler closure that takes an NSError object as an argument and returns no value. The handler is executed asynchronously.
///
/// \param receiver A type alias SystemEventReceived. SystemEventReceived is a type alias for an event handling closure that takes a WearableSystemEvent as a named argument and returns no value.
...
/// Enumeration representing status types for body activities.
typedef SWIFT_ENUM(NSInteger, WearableBodyActivityStatus) {

/// User has initiated a body activity.
  WearableBodyActivityStatusStarted = 0,

/// User's body activity is still in progress.
  WearableBodyActivityStatusOngoing = 1,

/// User has finished a body activity.
  WearableBodyActivityStatusFinished = 2,
};


/// Enumeration representing types of body activities.
...
///
/// \param found A type alias WearableScannerTokenFound. WearableScannerTokenFound is a type alias for a closure that takes a WearableScannerTokenResult object as an argument and returns no value. This closure is called after a wearable is found containing an SDK specific representation.
///
/// \param error A type alias WearableScannerError, a completion handler closure that takes an NSError object as an argument and returns no value. The handler is executed asynchronously.
///
/// \param finish A type alias WearableScannerFinish, a completion handler closure that takes no arguments and returns no value. The handler is executed asynchronously.
...
/// Enumeration representing types of system boot events.
typedef SWIFT_ENUM(NSInteger, WearableSystemEventBootType) {

/// A boot initiated as part of normal device operations.
  WearableSystemEventBootTypeMain = 0,

/// A boot initiated as part of an over the air device update.
  WearableSystemEventBootTypeOta = 1,

/// A boot initiated as part of a factory reset operation.
  WearableSystemEventBootTypeFactoryReset = 2,
};


/// Enumeration representing types of system shutdown events.
typedef SWIFT_ENUM(NSInteger, WearableSystemEventShutdownType) {

/// Shutdown is user initiated or occurred as part of normal device operations.
  WearableSystemEventShutdownTypeUser = 0,

/// Device shutdown due to critical battery state.
...
/// Describes a wearable user event.
...
/// Convenience initializer to create a wearable user event. - parameter gestureEvent: A WearableUserEventGesture optional. - parameter buttonEvent:  A WearableUserEventButton optional.
...
/// Represents a wearable user event button.
...
/// Property indicating the type of wearable user event button.
@property (nonatomic) enum WearableUserEventButtonType type;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end


/// Enumeration representing types of user button selection inputs. Currently, only single-press and double-press user button events are supported. Other types of user button event types may be implemented in a future release.
typedef SWIFT_ENUM(NSInteger, WearableUserEventButtonType) {

/// User taps on a button once.
  WearableUserEventButtonTypeSinglePress = 0,

/// User taps on a button two consecutive times.
  WearableUserEventButtonTypeDoublePress = 1,

/// Triple press user button events are not currently supported. This type of button event may be supported in a future release.
  WearableUserEventButtonTypeTriplePress = 2,

/// User presses a button using a long press, holding the button down for a moment in duration.
  WearableUserEventButtonTypeLongPress = 3,

/// User presses button but the type of press is not recognized.
...
/// Represents a wearable user event gesture.
...
/// Enumeration representing types of user gesture inputs.
typedef SWIFT_ENUM(NSInteger, WearableUserEventGestureType) {

/// User taps two consecutive times.
  WearableUserEventGestureTypeDoubleTap = 0,

/// User taps three consecutive times.
  WearableUserEventGestureTypeTripleTap = 1,

/// User taps an unknown number of times.
