...
def nominal_fota_flash(device, firmware_path):
	...
	if quark_id == firmware_id_to_flash:
		print '\nERROR: The Quark firmware (' + firmware_path + ') you try to flash is the same than the one running in the board'
		exit()
	...
	if pub == None:
		...
		if decodedResp.firmware_update_state != FirmwareUpdateState.NONE:
			...
			print ("------------------------------------------------")
			print ("------  START REQUEST => RECOVERY REBOOT  ------")
			print ("------------------------------------------------")
		else:
			print ("----------------------------------------------------------------")
			print ("------  DEVICE READY => START REQUEST => RECOVERY REBOOT  ------")
			print ("----------------------------------------------------------------")

	# BLE max message size is 132 bytes, the BLE message contain header + topic + req_id + protobuf serialization
	# protobuf serialization contain : command (start, cancel continue) + firmware size + data + offset
	...
	if quark_id_new != quark_id:
		print "TEST OK: firmware id found are different"
...
if __name__ == "__main__":
	...
	nominal_fota_flash(device, sys.argv[2])
