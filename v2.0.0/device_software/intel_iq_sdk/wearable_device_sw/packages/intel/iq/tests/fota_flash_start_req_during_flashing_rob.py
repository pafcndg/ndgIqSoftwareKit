...
def start_req_during_flashing(device, firmware_path):
	...
	while 1:
		...
		if i == 15:
			print ("----------------------------------------------------")
			print ("------  START REQUEST AFTER CONTINUE REQUEST  ------")
			print ("----------------------------------------------------")
			notif.firmware_update_cmd = FirmwareUpdate.START
			# Publish to intel/core/FirmwareUpdate and Wait response
			pub = device.itm_topic_req("\x49\x43\x46\x00", notif.SerializeToString())
			...
			if pub == None:
				...
				decodedResp = FirmwareUpdateState()
				decodedResp.ParseFromString(pub['data'])
				print("Response value = %d (expected: %d)"%(decodedResp.firmware_update_state, FirmwareUpdateState.IN_PROGRESS))
				assert pub['status'] == 0
				assert decodedResp.firmware_update_state == FirmwareUpdateState.IN_PROGRESS
			#set again CONTINUE request to keep going flashing
			notif.firmware_update_cmd = FirmwareUpdate.CONTINUE
...
if __name__ == "__main__":
	...
	start_req_during_flashing(device, sys.argv[2])
