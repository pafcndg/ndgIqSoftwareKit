    ...
    while index < len(data):
        if data[index:index+4] == "Pnc!":
            try:
                ...
                panics.append({"index":index, "memory":data[index:index+length]})
            index += length
        else:
            index += 4
    return panics
...
if __name__ == "__main__":
    ...
    if os.path.isfile(args.panic_content):
        ...
        panics = [{"index":0, "memory":''.join([chr(int(X, 16)) for X in [args.panic_content[start:start+2] for start in range(0, len(args.panic_content), 2)]])}]

    print("Number of panics found in binary: {:d}".format(len(panics)))
    for panic in panics:
        try:
            panic_data = DecodePanic(args.elf_path, panic["memory"])
            print panic_data
        except Exception as e:
            print("Failed parsing panic found at index 0x{:x} : {:s}".format(panic["index"], type(e)))
            traceback.print_exc()
