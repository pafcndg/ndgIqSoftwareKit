...
#include <fs/fat/ff.h>			/* Declarations of FatFs API */
#include <fs/fat/diskio.h>		/* Declarations of disk I/O functions */
#include <stdio.h>
#include <string.h>
...
static
int memcmp (const void* dst, const void* src, UINT cnt) {
		...
		if (!dp->clust) {		/* Static table */
			...
			if (((i / (SS(dp->fs) / SZ_DIRE)) & (dp->fs->csize - 1)) == 0) {	/* Cluster changed? */
				...
				if (clst >= dp->fs->n_fatent) {					/* If it reached end of dynamic table, */
					...
					memset(dp->fs->win, 0, SS(dp->fs));		/* Clear window buffer */
	...
	do {
		...
		if (c == DDEM || ((a & AM_VOL) && a != AM_LFN)) {	/* An entry without valid data */
			...
			if (a == AM_LFN) {			/* An LFN entry is found */
				...
				if (!(dp->fn[NSFLAG] & NS_LOSS) && !memcmp(dir, dp->fn, 11)) break;	/* SFN matched? */
				ord = 0xFF; dp->lfn_idx = 0xFFFF;	/* Reset LFN sequence */
			}
		}
